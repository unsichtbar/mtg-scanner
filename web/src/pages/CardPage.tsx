    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
      <div className="flex gap-4 p-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {selected.manaCost && <p className="text-xs text-slate-400 mt-1">{selected.manaCost}</p>}
...
CardLookup.Prices = function Prices({ prices }: { prices: Record<string, string | null> }) {
  ...
  return (
    <div className="flex gap-3 mt-2">
      {usd && <span className="text-xs text-slate-600">${usd} <span className="text-slate-400">non-foil</span></span>}
      {foil && <span className="text-xs text-slate-600">${foil} <span className="text-slate-400">foil</span></span>}
    </div>
  )
}
