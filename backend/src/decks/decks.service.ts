...
export class DecksService {
  constructor(
    ...
    private readonly decks: EntityRepository<Deck>,
    @InjectRepository(DeckCard)
    private readonly deckCards: EntityRepository<DeckCard>,
  ...
  async create(userId: string, name: string, format: GameFormat) {
    ...
    this.em.persist(deck); await this.em.flush();
    return deck;
  }
  ...
  async addCard(userId: string, deckId: string, cardId: string, quantity: number) {
    ...
    const existing = await this.deckCards.findOne({ deck, card });
    if (existing) {
    ...
    this.em.persist(deckCard); await this.em.flush();
    return deckCard;
  }

  async removeCard(userId: string, deckId: string, cardId: string) {
    const deck = await this.ensureOwner(userId, deckId);
    const deckCard = await this.deckCards.findOne({ deck, card: cardId });
    if (!deckCard) throw new NotFoundException('Card not in deck');
    this.em.remove(deckCard); await this.em.flush();
  }
  ...
  async delete(userId: string, deckId: string) {
    const deck = await this.ensureOwner(userId, deckId);
    this.em.remove(deck); await this.em.flush();
  }
